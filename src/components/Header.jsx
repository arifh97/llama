  ...
  return (
    <>
      <header className={`heading ${scrolled ? 'position-fixed' : ''}`}>
        <Container>
          <div className='heading-wrap border border-2 border-black bg-white d-flex align-items-center justify-content-between'>
          <Logo />
            ...
            <ul className="d-lg-flex align-items-center">
              {navLinks.map((link, index) => (
                <li key={index} className='d-block'>
                  <Link style={{backgroundColor:link.color}} className="heading-link text-capitalize" to={link.to} spy={true} smooth={true} duration={600} offset={0} onClick={() => handleTouchStart(link.to)}>{link.btn_title}</Link>
                </li>
              ))}
            </ul>
          </nav>
          <div className="heading-actions d-flex align-items-center flex-wrap">
            <a href='#' target='_blank' className='btn'>JOIN LLAMA</a>
